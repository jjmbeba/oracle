...
import { createChangeReportJob } from "./change-reports";
...
describe("change report job", () => {
  ...
  it("creates a job with the correct name and interval", () => {
    ...
    const job = createChangeReportJob({ db: mockDb, logger });

    expect(job.name).toBe("change-reports");
